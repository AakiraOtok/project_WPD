...
class FPNConvolutions(nn.Module):
    ...
    def __init__(self):
        ...
        self.fp4_conv     = nn.Conv2d(in_channels=512, out_channels=256, kernel_size=1)

        self.fp3_upsample = nn.Upsample(scale_factor=1.9, mode="bilinear")
        self.fp3_conv     = nn.Conv2d(in_channels=1024, out_channels=256, kernel_size=1)

        self.fp2_upsample = nn.Upsample(scale_factor=2, mode="bilinear")
        self.fp2_conv     = nn.Conv2d(in_channels=512, out_channels=256, kernel_size=1)
    ...
    def forward(self, conv3_3_feats, conv4_3_feats, conv7_feats, conv8_2_feats, conv9_2_feats, conv10_2_feats ,conv11_2_feats):
        ...
        out = F.relu(out + self.fp6_conv(conv10_2_feats))
        fp6_feats = out

        out = self.fp5_upsample(out)
        out = F.relu(out + self.fp5_conv(conv9_2_feats))
        fp5_feats = out

        out = self.fp4_upsample(out)
        out = F.relu(out + self.fp4_conv(conv8_2_feats))
        fp4_feats = out

        out = self.fp3_upsample(out)
        out = F.relu(out + self.fp3_conv(conv7_feats))
        fp3_feats = out

        out = self.fp2_upsample(out)
        out = F.relu(out + self.fp2_conv(conv4_3_feats))
        fp2_feats = out

        out = self.fp1_upsample(out)
        fp1_feats = F.relu(out + self.fp1_conv(conv3_3_feats))
